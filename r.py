...
# ── Stage 0 (NEW): Court ROI from green connected component ───────────────────
...
def detect_court_roi(frame: np.ndarray,
    ...
    largest green region in the lower part of the frame.

    Returns a tuple (roi_loose, roi_tight, roi_core):
      • roi_loose: convex hull of the green region dilated by `dilate_px`
        — used to mask the binary line image, so boundary lines (which
        sit on or just outside the green edge) are still included.
      • roi_tight: hull dilated by ~⅓ of `dilate_px` — used as a soft
        validity check.
      • roi_core: the un-dilated convex hull of the green region — used
        to verify that a candidate line's *supporting* white pixels lie
        on the playing surface, not in the magenta padding strip.
    ...
    mask = cv2.inRange(hsv, np.array([35, 40, 35]), np.array([95, 255, 255]))
    ...
    # Re-merge directly-adjacent green components (the net often splits
    # the court into two CCs; banners always have a positive vertical gap).
    ...
    # Convex hull of the largest contour — a real court is a convex
    # quadrilateral so the hull is a tight, well-behaved enclosing shape.
...
def filter_lines_by_support(lines: list, white_pts: np.ndarray,
                            ...
                            min_support: int = 80,
                            band_px: float = 4.0,
                            min_core_ratio: float = 0.5) -> list:
    """
    Keep only lines that satisfy BOTH:
      ...
      • At least `min_core_ratio` of those supporting pixels lie inside
        `roi_core` — the *un-dilated* convex hull of the green court.

    Why the core check matters: a line that runs through the magenta
    padding strip (parallel to a real sideline but ~30 px outside it)
    can collect hundreds of "white" pixels from edge artefacts in the
    padding, so a simple support-count test isn't enough.  Requiring
    the support to lie on the actual playing surface — which is GREEN,
    not magenta — eliminates that whole class of phantom sidelines.
    """
    ...
    for line in lines:
        ...
        dists = np.abs(dp[:, 0] * line.dv[1] - dp[:, 1] * line.dv[0])
        near = white_pts[dists < band_px]
...
def _score_homography(H: np.ndarray, all_lines: list,
                      ...
                      tol: float = 0.8) -> float:
    """
    Original line-alignment score, plus a multiplicative ROI-overlap factor:
    the fraction of the projected court that lies within the green ROI.
    """
    if H is None:
        return 0.0
    Hi = np.linalg.inv(H)
    ...
    if roi_mask is not None and img_shape is not None:
        ...
        if corners is not None:
            ...
            if quad_area > 0:
                ...
                overlap = inter / quad_area

    return align * (0.2 + 0.8 * overlap)
...
def detect_from_frame(frame: np.ndarray):
    ...
    # ★ Stage 0: court ROI from largest green CC.  Three masks:
    #   roi_loose — for masking the binary line image
    #   roi_tight — soft validity check (unused at the moment)
    #   roi_core  — un-dilated court, used to verify line support pixels
    ...
    # ★ Restrict to the loose ROI for line detection.
    if roi_loose is not None:
        filtered = cv2.bitwise_and(filtered, roi_loose)

    # 2. White pixels for line refinement
    ...
    # ★ Reject lines whose supporting pixels lie mostly OUTSIDE the
    # un-dilated court (i.e. in the magenta padding).  This is the
    # check that finally throws away the parallel-but-shifted phantom
    # sidelines that were beating the real ones on span.
    ...
    # 4. Fit. The loose mask is used in the validity / overlap checks.
    ...
    return out_pts, {
        ...
        "score": score, "roi_mask": roi_loose, "roi_core": roi_core,
        "H": H_mat,
    }
